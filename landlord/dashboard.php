    ...
    <?php foreach ($listings as $l): ?>
      <div class="listing-card">
        <div class="listing-photo" style="<?= $l['cover_photo'] ? "background-image:url('" . h(base_url($l['cover_photo'])) . "')" : '' ?>">
          ...
          <span class="status-badge status-<?= $l['status'] ?>"><?= h(ucfirst($l['status'])) ?></span>
        </div>
        <div class="listing-body">
          ...
          <div style="display:flex; gap:8px; margin-top:auto;">
            <a href="<?= base_url('landlord/edit_listing.php?id=' . $l['listing_id']) ?>" class="btn btn-ghost btn-sm" style="flex:1;">Edit</a>
            <a href="<?= base_url('boarder/view_listing.php?id=' . $l['listing_id']) ?>" class="btn btn-ghost btn-sm" style="flex:1;">View</a>
          </div>
          <form method="post" action="<?= base_url('landlord/delete_listing.php') ?>" onsubmit="return confirm('Delete this listing? This cannot be undone.');" style="margin-top:8px;">
            <?= csrf_field() ?>

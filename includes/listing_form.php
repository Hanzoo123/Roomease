...
<input type="text" id="name" name="name" value="<?= $val('name') ?>" placeholder="e.g. Greenview Student Dormitory"
  required>
...
<div class="field-row">
  <div>
    ...
    <input type="number" step="0.01" min="0" id="monthly_rent" name="monthly_rent" value="<?= $val('monthly_rent') ?>"
      placeholder="e.g. 3500.00" required>
  </div>
  <div>
    <label for="contact_number">Landlord contact number</label>
    <input type="text" id="contact_number" name="contact_number" value="<?= $val('contact_number') ?>"
...
<div class="field-row">
  ...
  <div>
    ...
    <input type="number" min="1" id="room_capacity" name="room_capacity" value="<?= $val('room_capacity', 1) ?>"
      required>
  </div>
</div>
...
<p class="field-hint" style="margin-top:-4px; margin-bottom:12px;">Select all amenities currently accessible to
  boarders:</p>
<div class="checkbox-grid"
  style="display:grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap:8px; margin-bottom:20px;">
...
<p class="field-hint" style="margin-top:-4px; margin-bottom:14px;">Define billing arrangements for each utility
  provided:</p>
...
<div style="display:flex; flex-direction:column; gap:12px; margin-bottom:24px;">
  <?php foreach (utility_options() as $u):
    ...
    ?>
    <div
      style="display:grid; grid-template-columns: 180px 1fr; gap:12px; align-items:center; background:rgba(0,0,0,0.02); padding:10px 14px; border-radius:6px; border:1px solid var(--board-border, #e2e8f0);">
      ...
      <div>
        <input type="text" name="billing_policy[<?= (int) $uId ?>]" value="<?= h($currentPolicy) ?>"
          placeholder="e.g. Included in Rent, Separate Meter, Split Equally..." list="policy-presets"
          style="margin:0; font-size:13px; height:36px;">
      </div>
    </div>
  <?php endforeach; ?>
...
<p class="field-hint" style="margin-top:-4px; margin-bottom:12px;">Upload high quality images of the room, facade, or
  facilities (JPG, PNG, WEBP — max 5MB each):</p>

<?php if (!empty($existingImages)): ?>
  <div style="display:flex; flex-wrap:wrap; gap:12px; margin-bottom:16px;">
    <?php foreach ($existingImages as $img): ?>
      <div
        style="position:relative; width:120px; height:90px; border-radius:6px; overflow:hidden; border:2px solid <?= $img['is_primary'] ? '#28a745' : '#cbd5e1' ?>; background:#f1f5f9;">
        <img src="<?= h(base_url($img['image_path'])) ?>" alt="Photo" style="width:100%; height:100%; object-fit:cover;">
        <?php if ($img['is_primary']): ?>
          <span
            style="position:absolute; top:4px; left:4px; background:#28a745; color:#fff; font-size:10px; font-weight:bold; padding:2px 6px; border-radius:3px;">Cover</span>
        <?php endif; ?>
      </div>
    <?php endforeach; ?>
  </div>
